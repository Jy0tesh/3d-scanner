...
int main(int argc,const char** argv)
{
	...
	if(!cap.isOpened())  // check if we succeeded
        return -1;
    Mat back,back3,otsuback1(frame.rows,frame.cols,CV_8UC1),otsuback2(frame.rows,frame.cols,CV_8UC1);				
    cap >> back;			//getting background
    cvtColor(back,backgray,COLOR_BGR2GRAY);
    cvtColor(back,backhsv,COLOR_BGR2HSV);          
    while(i!=frame.rows)
    {
        Mat back1=backgray(Range(i,i+frame.rows/4),Range(0,frame.cols));
        vector<Mat> channels(3);
        split(backhsv(Range(i,i+frame.rows/4),Range(0,frame.cols)),channels);
        ...
        otsuback1(Range(i,i+frame.rows/4),Range(0,frame.cols)) = back1;       // storing the slices in otsuback1,otsuback2
        otsuback2(Range(i,i+frame.rows/4),Range(0,frame.cols))=back2; 
        i=i+frame.rows/4;
